...
const AdminDashboard = () => {
  ...
  const navManagement = [
    ...
    ...(canManageUsers ? [{ name: 'Users', href: 'users', icon: MdPeople }] : []),
      ...
      <div className={`${sidebarOpen ? 'w-64' : 'w-16'} bg-white shadow-sm transition-all duration-300 ease-in-out border-r border-gray-200`}>
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 px-2 py-3 space-y-6">
            <div>
              {sidebarOpen && (
                <p className="px-3 mb-2 text-[11px] font-semibold tracking-wide text-gray-500 uppercase">Management</p>
              )}
            ...
            <div>
              {sidebarOpen && (
                <p className="px-3 mb-2 text-[11px] font-semibold tracking-wide text-gray-500 uppercase">Insights & System</p>
              )}
              <div className="space-y-1">
                {navSystem.map((item) => (
                  <button
