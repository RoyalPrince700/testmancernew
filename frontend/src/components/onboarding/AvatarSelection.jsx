  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600">Choose an avatar and create a unique username</p>
      </div>

      {/* Avatar Selection */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-3 text-center">Choose your avatar</label>
        <div className="grid grid-cols-6 gap-3 mb-6">
          {avatarOptions.map((avatarOption) => {
            const IconComponent = avatarOption.icon;
            return (
              <button
                key={avatarOption.emoji}
                onClick={() => handleAvatarSelect(avatarOption.emoji)}
                className={`text-4xl p-4 rounded-lg border-2 transition-colors ${
                  formData.avatar === avatarOption.emoji
